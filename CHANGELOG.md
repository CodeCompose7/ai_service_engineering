...
단위 시간 재배분. 클립 길이 추정을 다시 잡아 총량을 480분(8시간) → 432분(7시간 12분)으로
조정하고, 모든 문서의 시간 표기를 맞췄습니다.

### 변경됨 1.2.0

- **단위 시간 재배분** — `vod_plan.md` §3 단위별 분을 다시 산정. 섹션 합계 S1 120→134,
  S3 142→109, S4 123→100, S5 37→31(S2 58 유지), 총량 480분(8시간) → 432분(7시간 12분)
- 시간 표기 일괄 동기화 — vod_plan 요약표·헤더·합계 줄, 섹션 README 단위표·합계,
  각 lec 머리말 분량을 새 값과 정합

### 문서 1.2.0

- 루트 `README.md` 커리큘럼 요약을 현재 플랜 구조로 동기화 — 분뿐 아니라 옛 스냅샷이던
  구성까지 정리(6개 섹션/30단위·별도 캡스톤 행 → 5개 섹션/32단위, 캡스톤은 S5 흡수)
