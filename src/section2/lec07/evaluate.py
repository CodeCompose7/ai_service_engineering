...
- Recall@k: 상위 k개 안에 정답 근거가 들어 있으면 1, 아니면 0. 평균이 적중률이다.
...
def evaluate_config(eval_set, chunk_size: int, overlap: int, k: int = K) -> dict:
    """한 청킹 설정으로 인덱스를 만들고 평가셋을 검색해 Recall@k와 MRR을 잰다."""
    ...
    return {"chunks": len(chunks), "recall": mean(recalls), "mrr": mean(rrs)}
