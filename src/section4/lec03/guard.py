...
# 모델이 자연어 요청을 분류할 수 있는 행동 이름들. delete_account는 알지만 허용하지는 않는다.
KNOWN_ACTIONS = ["lookup", "summarize", "recommend", "delete_account"]
ACTION_ROUTER = (
    "사용자 요청을 다음 행동 중 하나로 분류해라: "
    "lookup, summarize, recommend, delete_account. 행동 이름 한 단어로만 답해라."
)
...
async def decide_action(request: str) -> str:
    """자연어 요청을 행동 이름으로 분류한다. 자연어를 행동으로 옮기는 건 모델이 한다."""
    raw = (await acomplete([
        {"role": "system", "content": ACTION_ROUTER},
        {"role": "user", "content": request},
    ])).lower()
    return next((action for action in KNOWN_ACTIONS if action in raw), raw.strip())


def check_action(action: str) -> None:
    """허용 목록에 없는 행동이면 막는다. action은 사용자 입력이 아니라 모델이 고른 행동이다."""
...
def main() -> int:
    print("=== 허용 행동 제약 (자연어 → 모델이 행동으로 분류 → 가드) ===")
    for request in ["내 정보 좀 찾아줘", "내 계정을 삭제해줘"]:
        action = asyncio.run(decide_action(request))
        try:
            check_action(action)
            print(f"  '{request}' → 행동 '{action}': 허용")
        except GuardError as exc:
            print(f"  '{request}' → 행동 '{action}': 막힘 ({exc})")
