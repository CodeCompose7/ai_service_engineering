...
호출은 S1·S2처럼 LiteLLM을 경유한다. tool calling의 신뢰성은 모델마다 달라(약한 모델은
lec07에서 다룬다), 여기서는 .env의 DEFAULT_PROVIDER가 가리키는 모델을 그대로 쓴다.
...
def chat(question: str, max_steps: int = 5) -> dict:
    ...
    llm_calls에는 이 질문을 처리하며 일어난 LiteLLM 호출 수가 담긴다. 도구 안에서 일어난
    호출(예: search_wikipedia의 요약)까지 같은 카운터로 세므로 모두 포함된다.
    """
    reset_calls()
