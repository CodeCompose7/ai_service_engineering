...
세 함수는 await할 I/O가 없지만, 도구 인터페이스를 맞추려고 async로 둔다.
"""
...
async def find_user(name: str) -> dict:
    """사용자 이름으로 사용자 id를 찾는다. 주문 조회의 첫 단계."""
    uid = _USERS.get(name) or _USERS.get(name.lower())
    return {"user_id": uid} if uid else {"error": f"'{name}' 사용자를 찾지 못했습니다."}
...
async def get_order_detail(order_id: str) -> dict:
    ...
    return detail if detail else {"error": f"주문 {order_id}를 찾지 못했습니다."}
