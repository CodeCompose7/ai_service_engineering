...
| 예제 도메인 | 위키백과 "검색 증강 생성" 문서 (rag.pdf · rag.html) |
...
| Ollama 로컬 | 5 | LiteLLM 백엔드 하나로, 호스트 연결 | 로컬 호출 예제 |
| 구조화 출력 1 | 22 | Pydantic 스키마, 프롬프트로 JSON 받기의 함정 | Pydantic 모델 |
| 구조화 출력 2 | 21 | instructor로 검증·재시도 | 안전한 추출 함수 |

### S2 — 데이터 & RAG 코어 (58분 / 7단위)

RAG에 필요한 데이터 처리부터 검색·평가까지 mini RAG 한 바퀴.

| 단위 | 분 | 핵심 내용 | 산출물 |
| --- | --- | --- | --- |
| 데이터 수집·정제 | 8 | CSV/웹/API 수집 + pandas 경량 정제 | 정제 스크립트 |
| 문서 로딩 | 6 | PyMuPDF로 PDF·HTML 로딩 + 한국어 함정 | 텍스트 추출기 |
| 청킹 | 7 | RecursiveCharacterTextSplitter, overlap, 의미 단위 | 청킹 유틸 |
| 임베딩 | 8 | sentence-transformers(HF) `bge-m3`, 유사도 직관 | 임베딩 함수 |
| 벡터DB Chroma | 10 | add/query, 메타데이터 필터 | Chroma 컬렉션 |
| mini RAG | 10 | retrieval → generation → 출처 표시 (rag.pdf) | 동작 mini RAG |
| 검색 평가 | 9 | Hit Rate@k/MRR, 청킹·임베딩 조합 비교 | 평가 스크립트·웹 |

### S3 — Agent 구성: 빌드 (138분 / 6단위)

도구를 쓰는 에이전트를 "동작하게" 만드는 빌드 레이어. 모든 호출은 LiteLLM 경유이며, Ollama 로컬 실행 시 tool calling 지원 모델을 쓰고 미지원 모델은 JSON 모드로 폴백한다.

| 단위 | 분 | 핵심 내용 | 산출물 |
| --- | --- | --- | --- |
| function calling 원리 | 24 | tool schema, 모델이 호출 결정하는 loop | 단일 tool 호출 |
| 단일 도구 에이전트 | 18 | 한 도구로 end-to-end | 동작 에이전트 |
| multi-tool agent | 27 | 도구 여러 개 라우팅 | 멀티툴 에이전트 |
| LangGraph 기초 | 22 | 상태·노드·엣지 개념 | 최소 그래프 |
| LangGraph 실전 | 28 | 분기/루프 있는 흐름 (도구 호출 자동화 예) | 자동화 그래프 |
| provider-agnostic 에이전트 | 19 | LiteLLM로 백엔드 교체, Ollama tool calling + JSON 모드 폴백 | 클라우드·Ollama 양쪽 동작 |
...
| 단위 | 분 | 핵심 내용 | 산출물 |
| --- | --- | --- | --- |
| 컨텍스트 엔지니어링 | 22 | 윈도우에 무엇을 언제 넣나, retrieval/메모리/compaction | 컨텍스트 조립 패턴 |
| 하네스 엔지니어링: 모델 + 하네스 | 22 | 제어 루프, 도구 인터페이스 설계, 능력 감지·우아한 강등(로컬 모델 폴백), "실패는 시스템 문제" | 최소 하네스 |
| 메모리·상태·가드레일 | 19 | 세션 간 상태 지속, 허용 행동 제약, 출력 검증·PII | 상태/가드 모듈 |
| 프롬프트 주입 방어 | 14 | 직접·간접 주입, 도구 결과 오염, 방어 패턴 | 방어 체크리스트 |
| 평가·검증·관찰 | 25 | LLM-as-judge, 트레이싱, 테스트셋·품질·개선 로그·한계 분석 | 평가 하네스 |
...
- 예제 데이터: 위키백과 "검색 증강 생성" 문서 (rag.pdf, rag.html)
