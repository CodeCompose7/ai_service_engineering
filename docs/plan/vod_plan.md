...
### S4 — 컨텍스트 & 하네스 엔지니어링: 신뢰성 (123분 / 7단위)
...
| 운영 신뢰성: 비용·레이트리밋·회복력 | 20 | 토큰·비용 예산, 프롬프트 캐싱, 레이트리밋(429)·재시도·백오프·타임아웃, 폴백 체인 | 운영 신뢰성 모듈 |
| 평가·검증 | 14 | LLM-as-judge, 테스트셋·품질 점수, 개선 로그·한계 분석 | 평가 하네스 |
| 관찰·운영 | 12 | 트레이싱, 구조화 로그, 메트릭·모니터링 | 관찰 모듈 |
...
| 통합 웹 서비스 (채팅 + 관리자) | 27 | RAG+가드+관찰을 한 어시스턴트로 조합, FastAPI가 채팅·관리자 페이지 제공, Docker·키 전략 | 통합 데모 레포 |
...
| S2 데이터 & RAG 코어 | 58 | 7 |
| S3 Agent 빌드 | 142 | 7 |
| S4 컨텍스트 & 하네스 (신뢰성) | 123 | 7 |
| S5 서빙 & 배포 (통합 데모 겸 캡스톤) | 37 | 2 |
| **합계** | **480** | **32** |
