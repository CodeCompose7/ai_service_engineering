...
위와 똑같이 입력 2,000 / 출력 500 토큰을 쓰는 호출을 저렴한 소형 모델과 고성능 대형 모델로 각각 돌리면 이렇게 갈립니다.

| 모델 등급 | 입력 단가(예시) | 출력 단가(예시) | 같은 호출 비용 |
| --- | --- | --- | --- |
| 소형·저렴 | 100만 토큰당 $0.15 | 100만 토큰당 $0.60 | 약 $0.0006 |
| 대형·고성능 | 100만 토큰당 $2.50 | 100만 토큰당 $10.00 | 약 $0.01 |

같은 일을 시켜도 모델 선택만으로 비용이 십수 배 벌어집니다. 그래서 작업 난이도에 맞는 모델을 고르는 것이 비용의 큰 축입니다. 단순한 분류나 추출에는 소형 모델로 충분한 경우가 많고, 어려운 추론에만 고성능 모델을 씁니다. 이 선택을 코드 변경 없이 모델 문자열만 바꿔 시험하는 방법은 lec06에서 다룹니다.
