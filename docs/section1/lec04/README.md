...
멀티턴에는 두 가지가 따라옵니다. 하나는 모델이 기억하지 않는다는 점, 다른 하나는 그래서 쌓이는 대화를 윈도우 안에서 줄이는 일입니다.
...
- 지난 user·assistant 메시지를 함께 실어 보내면 그제야 "Alice"라고 답합니다. lec02에서 본 "멀티턴 맥락은 매번 다시 실어 보내는 입력"이 이렇게 동작합니다.

멀티턴이 어떻게 쌓이는지 그림으로 보면 이렇습니다. 매 호출마다 지난 대화 전체를 다시 싣고 끝에 새 질문을 붙입니다.

```mermaid
flowchart TB
  T1["턴 1 호출<br/>[system, user1]"] --> R1["응답 → assistant1"]
  R1 --> T2["턴 2 호출<br/>[system, user1, assistant1, user2]"]
  T2 --> R2["응답 → assistant2"]
  R2 --> T3["턴 3 호출<br/>[system, user1, assistant1, user2, assistant2, user3]"]
  classDef default rx:8,ry:8;
  classDef grow fill:#eaf2ff,stroke:#4a78c0;
  class T1,T2,T3 grow;
```

목록 안의 `assistant` 항목은 모델이 직전에 한 답입니다. 모델은 아무것도 저장하지 않으므로, 이 목록을 만들어 보내는 일은 전적으로 애플리케이션의 몫입니다.

#### 6.3.2. 컨텍스트 한도와 compaction

이 목록은 무한정 커질 수 없습니다. lec02에서 본 컨텍스트 윈도우에 한도가 있어, 대화가 길어지면 언젠가 넘칩니다. 그때는 더 싣지 못하므로 오래된 대화를 잘라내거나 요약해 줄입니다. 이를 compaction이라 하며, 본격적인 처리는 S4 컨텍스트 엔지니어링에서 다룹니다.

```mermaid
flowchart LR
  H["쌓인 대화<br/>점점 길어짐"] --> Q{"윈도우에 들어가나?"}
  Q -->|"예"| SEND["전부 그대로 전송"]
  Q -->|"아니오"| CUT["오래된 턴 잘라내기"]
  Q -->|"아니오"| SUM["오래된 대화 요약<br/>compaction"]
  CUT --> SEND
  SUM --> SEND
  classDef default rx:8,ry:8;
```

매 호출이 전체 히스토리를 다시 보내므로 대화가 길수록 호출당 입력 토큰과 비용이 함께 커집니다. 그래서 무엇을 남기고 무엇을 줄일지를 정하는 것이 실무 과제가 됩니다.

### 6.4. (선택) 호출 지연 비교

응답이 끝나기까지 걸리는 시간도 백엔드마다 다릅니다. 같은 호출을 여러 번 보내 평균 시간을 재면 클라우드와 로컬의 속도 차가 드러납니다. 시간이 남을 때 보면 됩니다.

```bash
uv run python src/section1/lec04/first_call.py --latency
```

```text
=== (선택) 호출 지연 비교 — 같은 호출의 평균 응답 시간 ===
  gemini   평균 2.08초   (gemini/gemini-2.5-flash)
  ollama   평균 18.05초   (ollama/gemma4:12b-mxfp8)
```

- 같은 호출인데 클라우드 gemini는 약 2초, 로컬 ollama는 약 18초가 걸렸습니다. 로컬 모델은 내 장비에서 도므로 GPU와 모델 크기에 따라 크게 달라집니다.
- lec02에서 본 지연이 실제 초 단위로 나타난 것입니다. 무료라고 공짜는 아니어서, 로컬은 비용 대신 시간을 치르고 클라우드는 빠른 대신 토큰 단가를 냅니다.
- 숫자는 장비와 시점에 따라 달라지므로 절대값보다 클라우드와 로컬의 경향만 챙기면 됩니다.
