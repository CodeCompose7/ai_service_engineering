...
기본값으로 실행하면 `DEFAULT_PROVIDER`가 가리키는 모델이 답합니다. 이 저장소를 만든 환경에서는 그 값이 `ollama`의 `gemma4:12b-mxfp8`이라, 인자 없이 부르면 로컬 모델이 답합니다.

```text
$ uv run python src/section1/lec03/ask.py "LiteLLM을 한 문장으로 설명해줘"
[ollama / ollama/gemma4:12b-mxfp8]
LiteLLM은 여러 LLM 제공자를 단일 인터페이스로 호출하게 해 주는 파이썬 패키지입니다.
```

다만 아래 두 실험은 클라우드의 gemini로 돌렸습니다. temperature 0에서 응답이 멈추는 로컬 모델 특성 때문에, 비교는 전 구간을 안정적으로 처리하는 클라우드 모델로 했습니다.
...
- 샘플링 파라미터는 다음 토큰을 고르는 방식을 조절하는 손잡이입니다. temperature는 분포의 날카로움, top_p는 누적 확률 컷, top_k는 개수 컷입니다.
- 세 손잡이 모두 좁히면 출력이 한 값으로 모이고 넓히면 갈립니다. 세기는 temperature가 가장 크고 top_p가 보조, top_k는 약합니다.
- 같은 파라미터라도 프로바이더마다 받는 것이 달라, LiteLLM을 거쳐도 top_k 제거나 temperature 보정 같은 손질이 필요합니다. top_k는 OpenAI가 아예 받지 않습니다.
- 실무에서는 temperature부터 만지고 필요하면 top_p를 더합니다. 정답이 분명한 작업은 낮게, 다양성이 필요한 작업은 높게 시작하되 무작정 높이면 출력이 망가집니다.
