...
예제의 모델 문자열 목록을 직접 바꿔봅니다. 줄 하나를 `ollama/gemma4:12b-mxfp8`로 바꿔 로컬 모델을 끼워 넣거나, 보조 프로바이더를 빼보면서, 코드의 다른 부분은 전혀 손대지 않고 모델만 갈아끼우는 경험을 합니다. 같은 작업에 어떤 모델이 토큰 대비 괜찮은지도 출력으로 비교해봅니다.
