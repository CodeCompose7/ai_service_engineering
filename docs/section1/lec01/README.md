...
#### 5.1.2. OpenAI Platform 화면으로 따라 하기

OpenAI는 무료 티어가 없어 크레딧을 충전해야 호출이 됩니다. 그래서 흐름이 Gemini와 조금 다릅니다. 프로젝트를 만들고 키를 발급해 `.env`에 넣는 1·2단계로 키는 준비되지만, 실제로 호출하려면 3단계에서 크레딧을 충전해야 합니다. 충전했다면 4단계로 프로젝트 예산 한도를 걸어 과금이 불어나는 것을 막아 둡니다. 화면은 시점에 따라 바뀌므로 버튼 위치가 달라도 흐름만 같으면 됩니다.

##### 5.1.2.1. 프로젝트 만들기

[platform.openai.com](https://platform.openai.com/)에서 좌상단 프로젝트 선택 메뉴를 열고 새 프로젝트를 만듭니다. 이름은 나중에 알아보기 쉬운 것으로 정합니다.

![프로젝트 선택 메뉴에서 새 프로젝트 만들기](img/OpenAI-New_Project_01.png)
*좌상단 프로젝트 메뉴를 열고 "Create project"를 누릅니다.*

![프로젝트 이름 지정 후 만들기](img/OpenAI-New_Project_02.png)
*프로젝트 이름을 입력하고 "Create"를 누릅니다.*

##### 5.1.2.2. API 키 발급해 `.env`에 넣기

만든 프로젝트에서 시크릿 키를 만들고 키 이름과 대상 프로젝트를 정한 뒤 생성합니다. 생성 직후 뜨는 창에서 키를 복사해 저장소 루트 `.env`의 `OPENAI_API_KEY`에 붙여넣습니다. 키는 이 창을 닫으면 다시 보이지 않으니 이때 복사해 둡니다.

![API Keys에서 새 시크릿 키 만들기](img/OpenAI-API_key_01.png)
*좌측 "API Keys"로 들어가 우상단 "Create new secret key"를 누릅니다.*

![키 이름·프로젝트 지정 후 키 만들기](img/OpenAI-API_key_02.png)
*키 이름을 정하고 대상 프로젝트와 권한을 고른 뒤 "Create secret key"를 누릅니다.*

![생성된 키 복사](img/OpenAI-API_key_03.png)
*"Copy"로 키를 복사합니다. 이 값은 창을 닫으면 다시 보이지 않습니다.*

![.env의 OPENAI_API_KEY에 붙여넣기](img/OpenAI-API_key_04.png)
*복사한 키를 저장소 루트 `.env`의 `OPENAI_API_KEY` 값으로 붙여넣습니다.*

##### 5.1.2.3. 크레딧 충전하기

OpenAI는 크레딧이 0이면 호출이 막히므로, 키만으로는 부족하고 크레딧을 충전해야 실제 호출이 됩니다. 홈 화면이나 결제(Billing) 화면에서 크레딧을 더합니다.

![홈 화면에서 크레딧 추가 열기](img/OpenAI-Purchase_01.png)
*홈 화면의 크레딧 잔액 카드에서 "Add credits"를 누릅니다. 지금은 잔액이 0달러입니다.*

![결제 화면에서 크레딧 추가 열기](img/OpenAI-Purchase_02.png)
*결제(Billing) 화면에서도 "Add to credit balance"로 같은 충전을 시작할 수 있습니다.*

![충전 금액·결제 수단 선택](img/OpenAI-Purchase_03.png)
*충전할 금액을 입력하고 결제 수단을 고른 뒤 "Continue"를 누릅니다. 최소 5달러부터 충전됩니다.*

![결제 확인](img/OpenAI-Purchase_04.png)
*충전 내역과 세금을 확인하고 "Confirm payment"를 누릅니다.*

![크레딧 잔액 충전 확인](img/OpenAI-Purchase_05.png)
*결제가 끝나면 크레딧 잔액이 충전된 금액으로 바뀝니다.*

##### 5.1.2.4. 프로젝트 예산 한도 걸기 (권장)

크레딧을 충전했다면 프로젝트 월 예산 한도를 정해 둡니다. 한도에 닿으면 사용이 멈춰, 실수로 과금이 불어나는 것을 막는 안전장치입니다.

![프로젝트 설정의 Limits에서 예산 설정 열기](img/OpenAI-limit_01.png)
*프로젝트 설정(Settings)의 "Limits" 탭에서 "Set budget"을 누릅니다.*

![월 예산 한도 입력 후 추가](img/OpenAI-limit_02.png)
*월 예산 한도 금액을 입력하고 "Add budget"을 누릅니다.*

![예산 한도가 적용된 상태 확인](img/OpenAI-limit_03.png)
*한도가 걸린 상태입니다. 사용액이 이 한도에 닿으면 호출이 멈춥니다.*

#### 5.1.3. Claude Console 화면으로 따라 하기

Claude도 무료 티어가 없어 크레딧을 충전해야 호출이 됩니다. OpenAI와 흐름이 비슷하지만 프로젝트 대신 워크스페이스로 사용 사례를 나눕니다. 워크스페이스를 만들고 키를 발급해 `.env`에 넣는 1·2단계로 키는 준비되고, 실제로 호출하려면 3단계에서 크레딧을 충전합니다. 충전했다면 4단계로 워크스페이스 지출 한도를 걸어 과금이 불어나는 것을 막아 둡니다. 화면은 시점에 따라 바뀌므로 버튼 위치가 달라도 흐름만 같으면 됩니다.

##### 5.1.3.1. 워크스페이스 만들기

[platform.claude.com](https://platform.claude.com/)에서 좌상단 워크스페이스 선택 메뉴를 열고 새 워크스페이스를 만듭니다. 이름은 나중에 알아보기 쉬운 것으로 정합니다.

![워크스페이스 선택 메뉴에서 워크스페이스 생성](img/Claude-New_Project_Workspace_01.png)
*좌상단 워크스페이스 메뉴를 열고 "워크스페이스 생성"을 누릅니다.*

![워크스페이스 이름 지정 후 만들기](img/Claude-New_Project_Workspace_02.png)
*워크스페이스 이름을 입력하고 "만들기"를 누릅니다.*

##### 5.1.3.2. API 키 발급해 `.env`에 넣기

대시보드에서 API 키를 만들고 키 이름과 대상 워크스페이스를 정한 뒤 생성합니다. 생성 직후 뜨는 창에서 키를 복사해 저장소 루트 `.env`의 `ANTHROPIC_API_KEY`에 붙여넣습니다. 키는 이 창을 닫으면 다시 보이지 않으니 이때 복사해 둡니다.

![대시보드에서 API 키 받기](img/Claude-API_key_01.png)
*대시보드 우상단의 "API 키 받기"를 누릅니다.*

![키 이름·워크스페이스 지정 후 키 생성](img/Claude-API_key_02.png)
*키 이름을 정하고 대상 워크스페이스를 고른 뒤 "API 키 생성"을 누릅니다.*

![생성된 키 복사](img/Claude-API_key_03.png)
*복사 버튼으로 키를 복사합니다. 이 값은 창을 닫으면 다시 보이지 않습니다.*

![.env의 ANTHROPIC_API_KEY에 붙여넣기](img/Claude-API_key_04.png)
*복사한 키를 저장소 루트 `.env`의 `ANTHROPIC_API_KEY` 값으로 붙여넣습니다.*

##### 5.1.3.3. 크레딧 충전하기

Claude는 크레딧이 0이면 호출이 막히므로, 키만으로는 부족하고 크레딧을 충전해야 실제 호출이 됩니다. 대시보드의 크레딧 잔액에서 자금을 더합니다.

![대시보드에서 자금 추가 열기](img/Claude-Purchase_01.png)
*대시보드의 크레딧 잔액에서 "자금 추가"를 누릅니다. 지금은 잔액이 0달러입니다.*

![충전 금액·청구 정보 입력](img/Claude-Purchase_02.png)
*충전할 금액을 고르고 청구지 주소와 결제 정보를 입력합니다. 최소 5달러부터 충전됩니다.*

![구매 확인](img/Claude-Purchase_03.png)
*총 결제 금액을 확인하고 "크레딧 구매"를 누릅니다. 충전 금액에 부가세 10%가 더 붙어 결제되므로, 5달러를 충전하면 실제로는 5.5달러가 청구됩니다.*

##### 5.1.3.4. 지출 한도 걸기 (권장)

크레딧을 충전했다면 워크스페이스 월간 지출 한도를 정해 둡니다. 한도에 닿으면 사용이 멈춰, 실수로 과금이 불어나는 것을 막는 안전장치입니다.

![관리 메뉴의 한도에서 한도 변경 열기](img/Claude-limit_01.png)
*좌측 "관리"의 "한도"로 들어가 지출 한도의 "한도 변경"을 누릅니다.*

![월간 지출 한도 입력 후 설정](img/Claude-limit_02.png)
*월간 지출 한도 금액을 입력하고 "한도 설정"을 누릅니다.*

![한도가 적용된 상태 확인](img/Claude-limit_03.png)
*한도가 걸린 상태입니다. 사용액이 이 한도에 닿으면 호출이 멈춥니다.*
