...
| [lec02](lec02/README.md) | 15 | 서빙 Docker + 배포 | 배포 가능한 이미지 |

합계 40분, 2단위입니다.

## 흐름

먼저 기능을 API로 감쌉니다. lifespan으로 자원을 띄우고, 엔드포인트로 요청을 받고, 입력을 검증하고, 응답을 스트리밍으로 흘려보냅니다. 그다음 그 서버를 Docker 이미지로 묶어, 개발 환경과 다른 점을 짚으며 클라우드에 출하합니다.
