...
한 가지 주의가 있습니다. 미리 빌드한 인덱스(`chroma_db`)는 이미지에 들어가지 않습니다. 용량이 크고 `.gitignore` 대상이라서요. 대신 원본 문서(`rag.pdf`)는 소스에 있어 함께 출하되고, `open_index`가 인덱스가 비어 있으면 그 문서로 다시 채웁니다. 그래서 배포된 컨테이너에서도 RAG는 돌지만, 첫 요청이 인덱스를 새로 빌드합니다. 임베딩 모델을 내려받고 문서를 다시 임베딩하느라 첫 요청이 느리고 디스크·네트워크가 듭니다. 인덱스를 미리 빌드해 함께 굽거나 볼륨으로 붙여 이 콜드 스타트를 없애는 것은 본격 서빙 주제라 트랙에서 다룹니다.
