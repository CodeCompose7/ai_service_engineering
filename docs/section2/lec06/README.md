...
top-k는 가장 가까운 하나만 보는 것과 다릅니다. 답이 여러 청크에 갈려 있으면 한 개로는 일부만 잡히지만, 상위 k개를 가져오면 갈린 조각이 함께 딸려 옵니다.

```mermaid
flowchart TB
  Q["질문"] --> S["유사도 검색"]
  S --> A["top-1<br/>가장 가까운 1개<br/>답이 갈리면 일부만"]
  S --> B["top-k<br/>상위 k개<br/>갈린 조각도 함께"]
  classDef default rx:8,ry:8;
```

무엇이 LLM에 들어가는지 예제로 보면 이렇습니다.

```mermaid
flowchart TB
  Q["질문"] --> R["검색 top-3<br/>#0 · #6 · #5 (유사도)"]
  R --> E["각 청크의 앞뒤 ±1을 모음<br/>합집합 #0·1·4·5·6·7 = 6청크"]
  E --> P["6청크의 텍스트만<br/>[1]~[6] 번호로 프롬프트에"]
  P --> L["LLM"]
  R -. 유사도 .-> U["순위·grounding·출처에만 사용<br/>프롬프트엔 안 들어감"]
  classDef default rx:8,ry:8;
```

청크 id가 `chunk_<번호>`라, 검색된 k개 청크 각각의 앞뒤 번호(±1)를 모아 합집합을 만들고 번호 순으로 잇습니다. 예제에서는 top-3(#0·#6·#5)가 각자 이웃을 더해 6청크(#0·1·4·5·6·7)가 됩니다. 한 청크만 늘리는 것이 아니라 검색된 여러 청크를 모두 넓힙니다. 프롬프트에는 이 청크들의 텍스트만 `[1]`~`[6]`으로 들어가고, 유사도 점수는 순위·grounding·출처 표시에만 쓰여 프롬프트에는 포함되지 않습니다.
...
```mermaid
flowchart LR
  C["근거 청크들"] --> NB["근거마다 번호 매기기"]
  NB --> UM["user 메시지<br/>근거 + 질문"]
  SP["system 메시지<br/>근거로만 답·출처 달기"] --> MSG["messages"]
  UM --> MSG
  MSG --> LLM["LLM"]
  classDef default rx:8,ry:8;
```
...
flowchart TD
  Q["질문"] --> S["검색 top-k"]
  S --> J{"관련 근거가 있나<br/>유사도가 충분한가"}
  J -->|"예 · 근거 있음"| A["근거로 답 + 출처 번호"]
  J -->|"아니오 · 유사도 낮음"| N["모릅니다<br/>지어내지 않음"]
  classDef default rx:8,ry:8;
```
...
아래는 함수 호출 구조입니다. 화살표는 호출 방향이지 실행 순서가 아닙니다. `answer()`가 retrieve·expand_with_neighbors·generate를 부르고, 검색과 확장은 lec05 crud를 거쳐 벡터DB에 닿습니다.
...
무엇을 얼마나 손봐야 하는지는 감이 아니라 측정으로 정합니다. 다음 단위 lec07에서 검색이 정답 근거를 잘 가져오는지 Hit Rate@k 같은 지표로 평가합니다.
