...
[load.py](../../../src/section2/lec02/load.py)는 일부러 함정을 심은 [sample.pdf](../../../src/section2/lec02/data/sample.pdf)를 읽습니다. 1쪽은 조문 형식의 한국어 사규라 줄을 접는 자리마다 줄바꿈이 들어가고, 2쪽은 1쪽을 이미지로 넣은 스캔본 흉내라 텍스트가 없습니다. 같은 추출기를 [sample.html](../../../src/section2/lec02/data/sample.html)에도 그대로 씁니다.
...
```text
=== 1. PDF 원시 추출 vs 정제 ===
1쪽 원시: '아크메 주식회사 환불 및 교환 규정\n제1조 (목적)\n이 규정은 아크메 주식회사가 판매하는 상품의 환' ...
  개행 13개 — 문장이 줄 단위로 끊깁니다
1쪽 정제: 아크메 주식회사 환불 및 교환 규정 제1조 (목적) 이 규정은 아크메 주식회사가 판매하는 상품의 환불과 교환에 관한 기준을 정 ...
...
=== 3. HTML도 같은 코드로 ===
  홈 · 제품 · 고객지원 · 로그인 아크메 주식회사 환불 규정 고객은 상품을 받은 날로부터 7일 이내에 환불을 요청할 수 있습
```

읽어낼 점입니다.

- 원시 추출에는 문장 중간에 개행이 13개 들어가 있습니다. 텍스트박스가 줄을 접은 자리입니다. `normalize`가 이를 공백으로 이어, 검색·청킹에 쓸 수 있는 연속 텍스트로 만듭니다.
- 2쪽은 0자입니다. 텍스트 레이어가 없는 이미지라, 추출로는 아무것도 나오지 않습니다. 빈 페이지로 표시해 OCR이 필요함을 알립니다.
- HTML도 같은 `extract_text`로 텍스트가 나옵니다. 다만 `홈 · 제품 · 고객지원 · 로그인` 같은 내비게이션·푸터까지 함께 들어옵니다. `<script>`는 빠지지만, 본문만 원하면 보일러플레이트 제거가 따로 필요합니다.
