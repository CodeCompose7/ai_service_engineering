...
이 추출은 LLM에 보내기 전의 전처리입니다. 문서를 통째로 LLM에 넣으면 컨텍스트 한계를 넘고 비용도 크며 보일러플레이트 같은 잡음까지 섞입니다. 그래서 깨끗한 텍스트로 만들어 두었다가, 뒤의 청킹·임베딩·검색으로 질문에 맞는 조각만 골라 LLM에 보냅니다. lec01에서 본 RAG의 인덱싱 단계가 여기서 시작됩니다.
...
자모 분리는 눈에 잘 안 띄지만 검색을 망칩니다. `검색`이 겉보기엔 같아도 자모로 쪼개져 저장되면, 합쳐진 글자와 다른 것으로 취급됩니다. `NFC`가 이를 한 글자로 합쳐, lec01의 텍스트 정제와 같은 결을 PDF에도 적용합니다.
...
스캔본은 다릅니다. 텍스트 PDF는 추출만으로 충분하지만, 텍스트 레이어가 없는 스캔·이미지 PDF는 OCR을 써야 글자가 나옵니다. 그래서 OCR은 모든 PDF가 아니라 빈 페이지에만 거는 조건부 단계입니다. PyMuPDF도 Tesseract가 깔려 있으면 `get_textpage_ocr`로 OCR을 합니다.
