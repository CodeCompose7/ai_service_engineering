...
질문마다 정답 근거가 검색 상위에 들어오는지를 봅니다. 두 지표를 씁니다.

- Recall@k: 상위 k개 안에 정답 근거가 있으면 1, 없으면 0입니다. 평균이 적중률입니다.
- MRR: 정답이 처음 나온 순위의 역수, 즉 1/순위입니다. 1위면 1.0, 3위면 0.33으로, 위쪽에 올수록 높습니다.
...
Recall@k는 "찾았나"를, MRR은 "얼마나 위에서 찾았나"를 봅니다. 둘을 함께 보면 검색이 정답을 빠뜨리지 않으면서 위쪽에 올리는지 알 수 있습니다.
