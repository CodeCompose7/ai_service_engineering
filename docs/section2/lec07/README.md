...
한 질문의 Recall@k는 상위 k개 안에 정답 근거가 있으면 1, 없으면 0입니다. 1위든 k위든 안에만 들면 똑같이 1이라, 위치는 보지 않고 "빠뜨리지 않고 데려왔나"만 봅니다.
...
정답이 3위에 있으면 k에 따라 점수가 달라집니다. Recall@1과 Recall@2는 0이고, 3위가 잡히는 Recall@3부터 1입니다. k를 키울수록 정답을 담기 쉽지만, 관련 없는 청크도 그만큼 함께 딸려 옵니다.

여러 질문의 이 0과 1을 평균낸 것이 Recall@k입니다. 5문항 중 4문항에서 정답이 상위 k에 들면 Recall@k는 0.80입니다.

### 2.2. MRR — 얼마나 위에서 찾았나

Recall이 "들었나"만 본다면, MRR은 "몇 위로 들었나"까지 봅니다. 한 질문의 점수는 정답 근거가 처음 나온 순위의 역수, 즉 1/순위입니다.
