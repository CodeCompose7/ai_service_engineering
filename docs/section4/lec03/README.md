...
| 부작용 있음 (메일·결제) | 반드시 체크포인트나 idempotency key | 안 그러면 메일·결제가 두 번 |

assistant.py의 스텝은 다 읽기성 LLM 호출이라 외부 부작용이 없습니다. 통째 재실행이 좀 낭비여도 깨지진 않습니다. 만약 여기 `send_email`이 끼면, 통째 재실행은 메일을 두 번 보내니 스텝별 체크포인트가 필요해집니다.
...
- 상태는 메모리(요청 간 맥락)로도, 체크포인트(작업 중 재개)로도 씁니다. 통째 재실행이 안전한지는 스텝의 멱등성에 달렸고, 부작용이 있으면 체크포인트로 중복을 막습니다. 스텝별 재개는 LangGraph가 자동으로 합니다.
