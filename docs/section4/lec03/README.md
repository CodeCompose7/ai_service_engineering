...
[assistant.py](../../../src/section4/lec03/assistant.py)의 `GuardedAssistant`가 이 흐름을 코드로 보입니다. state.py와 guard.py를 부품으로 가져와 한 번의 `handle`에 엮습니다. `handle` 한 번이 도는 과정은 이렇습니다. 상태를 불러오고, 행동을 분류해 검사하고, 막히면 거기서 멈춥니다. 허용되면 새 사실을 배우고, 모델을 돌려 답을 만들어 검증하고, PII를 가린 뒤, 바뀐 상태를 저장합니다.

```mermaid
flowchart TD
  H["handle(session_id, request)"] --> LOAD["상태 load · turns += 1"]
  LOAD --> ACT["decide_action<br/>자연어 → 행동"]
  ACT --> CHK{"check_action<br/>허용?"}
  CHK -->|"아니오"| REF["상태 save · 거부 반환"]
  CHK -->|"예"| EXT["_extract_facts<br/>새 사실 학습 → facts 갱신"]
  EXT --> RESP["_respond<br/>모델 실행 + 출력 검증"]
  RESP --> MASK["redact_pii<br/>PII 마스킹"]
  MASK --> SAVE["상태 save · 답 반환"]
  classDef default rx:8,ry:8;
```
...
## 6. 다시 실행하면 — 메모리와 체크포인트

`handle`이 중간에 죽고 다시 불리면 어떻게 될까요. 상태를 쓰는 방식이 두 가지라, 헷갈리기 쉬워 갈라 둡니다.

| 쓰임 | 무엇 | 이 단원 |
| --- | --- | --- |
| 메모리 | 요청 사이에 맥락을 나름 (어제 한 말을 오늘도) | assistant.py |
| 체크포인트 | 한 작업이 중단되면 그 자리부터 재개 | LangGraph (S3 lec05·06) |

assistant.py는 상태를 메모리로 씁니다. 그래서 매 `handle`은 통째로 다시 돕니다. 건너뛰는 스텝이 없습니다. `_respond`에서 죽고 다시 부르면 앞의 `_extract_facts`도 다시 불립니다. `save`가 맨 끝에만 있어, 중간에 죽으면 아무것도 저장되지 않고 처음부터 다시 합니다.

```mermaid
flowchart TB
  subgraph A["통째 재실행 (assistant.py)"]
    direction LR
    A1["load"] --> A2["extract"] --> A3["respond 죽음"]
    A3 -.다시.-> A1
  end
  subgraph B["체크포인트 재개 (LangGraph)"]
    direction LR
    B1["load 완료"] --> B2["extract 완료"] --> B3["respond 죽음"]
    B3 -.재개.-> B3
  end
  classDef default rx:8,ry:8;
```

이게 좋은지는 스텝의 성격에 달렸습니다. 핵심은 멱등성입니다. 두 번 실행해도 결과가 같고 무해하면 멱등합니다.

| 스텝 성격 | 권장 | 이유 |
| --- | --- | --- |
| 싸고 멱등 (읽기·계산) | 그냥 재실행 | 두 번 해도 무해·저렴, 체크포인트가 아깝다 |
| 비쌈 (긴 LLM 호출) | 스텝 뒤 체크포인트, 재개 시 skip | 비싼 호출을 또 치르지 않으려고 |
| 부수효과 있음 (메일·결제) | 반드시 체크포인트나 idempotency key | 안 그러면 메일·결제가 두 번 |

assistant.py의 스텝은 다 읽기성 LLM 호출이라 외부 부수효과가 없습니다. 통째 재실행이 좀 낭비여도 깨지진 않습니다. 만약 여기 `send_email`이 끼면, 통째 재실행은 메일을 두 번 보내니 스텝별 체크포인트가 필요해집니다.

"끝난 스텝은 건너뛰고 죽은 자리부터 잇기"가 바로 LangGraph가 노드마다 자동으로 하는 일입니다. S3 lec05·06에서 다룹니다. 체크포인트 입자가 고울수록 정확히 재개하지만 코드가 복잡해집니다.
...
- 상태는 메모리(요청 간 맥락)로도, 체크포인트(작업 중 재개)로도 씁니다. 통째 재실행이 안전한지는 스텝의 멱등성에 달렸고, 부수효과가 있으면 체크포인트로 중복을 막습니다. 스텝별 재개는 LangGraph가 자동으로 합니다.
