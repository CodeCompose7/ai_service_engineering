...
toy 지식 주입은 메커니즘을 보이기엔 좋지만 너무 쉽습니다. 0%와 100%처럼 극단적이죠. 진짜 평가는 실제 시스템에 합니다. [rag_eval.py](../../../src/section4/lec06/rag_eval.py)는 S2에서 만든 RAG(Chroma 인덱스 + RAG를 설명한 위키 문서)를 평가합니다. 핵심 질문은 "우리 RAG가 그냥 모델에게 묻는 것보다 나은가"입니다.

평가 부품은 그대로 둡니다. `EvalHarness`와 `judge`는 evaluate.py 것을 쓰고, `answer_fn`만 RAG와 모델 단독으로 갈아 끼웁니다. 저지·에이전트를 주입식으로 둔 설계 덕분입니다.
...
| 질문 | RAG | 모델 단독 |
| --- | --- | --- |
| RAG란 무엇인가요? | PASS | PASS |
| RAG라는 용어는 언제 어디서 처음 소개되었나요? | PASS | PASS |
| Retro 방식의 단점은 무엇인가요? | PASS | FAIL |
| 희소 벡터의 특징은 무엇인가요? | PASS | PASS |
| 점수 | 100% | 75% |

표가 어디서 RAG가 이기는지 보여줍니다. 일반적인 질문은 모델도 알아서 둘 다 통과합니다. 그런데 "Retro 방식의 단점"처럼 문서에만 있는 세부는 모델 단독이 모릅니다. RAG는 그 문서를 검색해 와 맞춥니다. RAG가 100%, 모델 단독이 75%로, 검색이 실제로 품질을 올린 것입니다.

이게 현실적인 평가입니다. 점수 차가 극단적이지 않고, 어떤 종류의 질문에서 차이가 나는지가 표에 드러납니다. 표 하나로 "RAG를 쓸 값어치가 있나"에 숫자로 답합니다.
...
- 진짜 평가는 실제 시스템에 합니다. S2 RAG를 모델 단독과 견줘 표로 내면, 검색이 값어치 있는지 숫자로 답합니다.
