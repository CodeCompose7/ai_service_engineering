...
알림은 운영의 한 조각일 뿐입니다. 운영에는 대시보드(메트릭을 계속 봄), SLO·SLA(목표를 정하고 추적), 사고 대응(터지면 로그·트레이스로 파고들어 고치고 회고), 안전한 배포(카나리로 조금 내보내고 메트릭 보다가 나빠지면 롤백), 온콜·런북이 다 들어갑니다. 대부분은 코드가 아니라 도구·프로세스입니다. Prometheus 알림 룰, PagerDuty, Grafana, CI/CD 카나리 같은 것입니다. 관찰이 그 모두의 토대입니다.
