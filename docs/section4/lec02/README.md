...
하네스가 맡는 일은 이렇습니다. 이 단원은 제어 루프·도구 인터페이스·능력 감지·강등·복구를 다루고, 상태·메모리·가드레일은 다음 단원들로 이어집니다.
...
폴백(우아한 강등)은 이 가운데 하나일 뿐입니다. 모델이 네이티브 도구 호출을 못 할 때 다른 방식으로 같은 일을 하게 하는 한 가지 책임이지, 하네스의 전부가 아닙니다. 하네스는 모델을 감싸 에이전트로 만드는 전체 장치이고, 폴백은 그 장치의 한 부품입니다.
