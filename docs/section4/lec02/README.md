...
같은 "하네스"라는 말이 세 층위로 쓰입니다. 헷갈리지 않게 갈라 둡니다.

| 레벨 | 무엇 | 예 |
| --- | --- | --- |
| 1 개념 | 모델을 감싸는 런타임 | 하네스 그 자체 |
| 2 짓는 법 | 직접 짜기 또는 프레임워크 | 우리 `Harness` / LangGraph·LangChain·OpenAI Agents SDK |
| 3 부품 | 한 슬롯을 채우는 라이브러리 | instructor(검증)·LiteLLM(모델)·MCP(도구) |

LangGraph는 하네스를 짓는 프레임워크(레벨 2)이고, 그 프레임워크로 만든 그래프가 하네스(레벨 1)가 됩니다. instructor는 검증 슬롯에 꽂히는 부품(레벨 3)이라 그 자체로는 하네스가 아닙니다. 우리 `Harness`는 손으로 짠 작은 하네스입니다. 그래서 "다 하네스"는 레벨 1과 2까지이고, 부품은 그 안에 꽂힙니다.
...
과제: 좋은 하루 보내는 법 하나만 알려줘
  답: 아침에 거울을 보고 활짝 웃어보세요! ...
  트레이스: ['입력 검열: 통과', '출력 검열: 통과']
  LLM 호출: 3회 (검열이 응답 호출에 더해진다)

과제: 야 이 쓰 레 기야
  차단: 부적절한 표현이 감지되어 차단했습니다
  트레이스: ['입력 검열: 차단']
  LLM 호출: 1회 (검열이 응답 호출에 더해진다)
```

"쓰 레 기"처럼 띄어 쓰면 regex 차단 목록은 그대로 지나치지만, LLM 검열은 뜻을 알아채 막습니다. 패턴으로 못 잡는 판단을 모델에게 맡기는 것입니다.

대신 LLM 호출이 늘어납니다. 통과하는 요청 하나에 입력 검열·응답·출력 검열로 3회를 부릅니다. 응답만 하면 1회면 될 일입니다. 차단되는 요청은 입력 검열 1회로 끝나 싸지만, 정상 요청일수록 호출이 배로 듭니다. 검열을 촘촘히 둘수록 호출 수와 지연·비용이 함께 오릅니다. regex 같은 공짜 판단과 달리, 똑똑한 판단에는 값이 붙는다는 뜻입니다. 비용·레이트리밋 관리는 lec05 운영 신뢰성에서 다룹니다.

LLM 검열 말고도 비-regex 방법이 있습니다. moderation API(OpenAI /moderations·litellm.moderation), 전용 독성 분류기(Detoxify), 임베딩 유사도로 시드 욕설과 거리 재기입니다. 셋 다 패턴이 아니라 의미·통계로 판단합니다. 비용을 줄이려고 실전에서는 값싼 regex로 먼저 거르고, 애매한 것만 LLM 검열로 넘기는 식으로 섞기도 합니다.
