...
다만 도구를 제때 정확히 부르는 능력은 모델마다 다릅니다. 로컬 소형 모델은 불필요한데도 도구를 부르거나, 결과를 받고도 깔끔히 답하지 못하기도 합니다. 그래서 기초인 이 단위는 안정적인 클라우드 프로바이더를 앞세웁니다. 로컬 모델에서 이걸 어떻게 다루는지는 lec06에서 봅니다.
