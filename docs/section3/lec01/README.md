...
끝에 찍힌 LLM 호출 수가 도구 사용의 값을 보여줍니다. 도구가 필요 없으면 1회로 끝나고(바로 답), 도구를 한 번 쓰면 2회입니다. 도구를 부르겠다고 정하는 호출 한 번, 도구 결과를 받아 답하는 호출 한 번입니다. 위키 질문은 3회인데, 바깥 loop의 2회에 더해 search_wikipedia가 안에서 요약·번역에 LLM을 한 번 더 쓰기 때문입니다. 도구를 쓸수록, 또 도구가 안에서 LLM을 쓸수록 호출이 늘어 비용과 시간이 커집니다.
