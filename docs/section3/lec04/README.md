...
우리 예제는 이 설정을 코드로 적었을 뿐입니다. agent.py의 `SERVERS` 리스트가 곧 `mcpServers`의 파이썬판으로, 서버마다 `command`와 `args`를 주는 똑같은 정보입니다. 다만 전송은 stdio 하나만 보였습니다. HTTP/SSE로 원격 서버에 붙는 것은 같은 흐름에 전송만 바꾸는 일이라, 개념으로만 짚고 넘어갑니다.
