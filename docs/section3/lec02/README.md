...
한 가지 분명히 할 점이 있습니다. 이 세 방식이 바꾸는 것은 시간뿐이고, LLM 호출 수는 그대로입니다. 호출 수는 에이전트 루프가 도구를 몇 번 부르고 그 결과로 몇 번 더 답하느냐로 정해집니다. 도구 결과를 받아 다시 부르면 늘어나지만, 같은 호출을 순차로 굴리든 동시에 굴리든 횟수는 같습니다. 곧 순차·병렬·비동기는 function calling이 아니라 파이썬 동시성 기법의 층이고, 도구를 sync로 짜느냐 async로 짜느냐의 문제입니다.
