...
| 섹션 | 주제 | 분 | 단위 |
| --- | --- | --- | --- |
| S1 | LLM을 서비스로 (멀티 프로바이더·프롬프트·구조화 출력) | 134 | 9 |
| S2 | 데이터 & RAG 코어 (수집·청킹·임베딩·검색 평가) | 58 | 7 |
| S3 | Agent 빌드 (function calling·multi-tool·LangGraph) | 109 | 7 |
| S4 | 컨텍스트 & 하네스 엔지니어링 (신뢰성) | 100 | 7 |
| S5 | 서빙 & 배포 (FastAPI → Render/Railway, 통합 데모 겸 캡스톤) | 31 | 2 |
| 합계 | | 432 | 32 |
...
예제 코드는 이 저장소로 공유됩니다. 강의는 코드를 한 줄씩 따라 치는 대신, 공유된 코드를 devcontainer 안에서 실행해 결과를 보고 핵심을 읽어 이해하는 방식으로 진행합니다. 직접 손대는 부분은 자기 키를 넣거나 프롬프트·스키마를 바꿔보는 것처럼 의미 있는 몇 군데로 한정하며, 각 단위 문서의 "직접 해보기"로 표시합니다.

## 강의별 코드 받기

강의 코드는 단위별 git 태그로 제공됩니다. 한 단위를 시작할 때 그 단위의 태그를 체크아웃하면, 그 강의까지의 완성된 코드를 그대로 받습니다.

```bash
git checkout s1-lec02   # S1 lec02까지의 코드
```

- 태그는 `s1-lec01`, `s1-lec02`처럼 `s{섹션}-lec{단위}` 형식이며, 각 태그는 이전 단위 코드를 포함한 누적 상태입니다.
- 일부 단위는 시작 자료가 필요해 `s1-lecNN-start` 태그를 함께 제공합니다. 그때는 `-start`에서 시작해 따라가고, 완성본은 `s1-lecNN`입니다.
